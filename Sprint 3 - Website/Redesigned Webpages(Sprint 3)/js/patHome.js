...
async function pullBook(){	
	...
	const db = await database.collection('TestingBookings').where("patientName", "==", name);
    ...
    .then((docSnapshot) => {
      if (docSnapshot.exists) {
        phoneCheck.onSnapshot((doc) => {       
          ...
          document.getElementById("PatNameHead").textContent = Patientdata['First Name']+' '+ Patientdata['Last Name'];
